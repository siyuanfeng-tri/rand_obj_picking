...
int main(int argc, char **argv) {
  ...
  X0.translation()[2] = 0;
  // robot_comm.MoveTool(X0, 3, 10, true);
  robot_comm.MoveStraightUntilTouch(
      Eigen::Vector3d::UnitZ(), -0.03,
      Eigen::Vector3d(0, 0, 10), true);

  // Push in.
  robot_comm.GetRobotState(&robot_state);
  X0 = robot_state.get_X_WT();
  X0.linear().setIdentity();
  X0.translation()[1] += 0.03;
  // robot_comm.MoveTool(X0, 3, 50, true);
  robot_comm.MoveStraightUntilTouch(
      Eigen::Vector3d::UnitY(), 0.03,
      Eigen::Vector3d(0, 20, 15), true);
  ...
  robot_comm.MoveTool(X0, 3, Eigen::Vector6d::Constant(50), true);
  ...
  // Reset.
  robot_comm.MoveJointDegrees(q2, 2, true);
